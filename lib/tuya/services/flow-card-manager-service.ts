...
/**
 * Structural view of the per-device ServiceCoordinator, limited to what the Flow
 * card handlers need. Declared structurally to avoid a circular import between
 * ServiceCoordinator and this service.
 */
interface DeviceServiceCoordinator {
  getAdaptiveControl?: () => {
    ...
    getBuildingModelService?: () => {
      getLearner: () => { getModel: () => { C: number; UA: number; confidence: number } };
    } | null;
    getExternalTemperatureService?: () => {
      getIndoorTemperature: () => number | null;
    } | null;
    receiveExternalTemperature?: (temperature: number) => Promise<void>;
  } | null;
  getBuildingInsights?: () => BuildingInsightsService | null;
  /**
   * NOTE: not implemented by ServiceCoordinator. RollingCOPCalculator is a private
   * field on the device class. Declared optional so callers must use `?.()` and
   * degrade gracefully instead of throwing. See ADR-001 follow-up.
   */
...
export class FlowCardManagerService {
  ...
  /**
   * The instance that registered the app-wide Flow card listeners.
   *
   * Flow cards are app-wide singletons and the Homey SDK offers no way to remove a
   * run listener, so registering from every device instance is redundant: the last
   * call simply overwrites the previous one. One registrar serves all devices
   * because every handler resolves its target through `args.device`.
   *
   * Released in destroy() so a remaining device takes over when the registrar's
   * device is removed.
   */
  ...
  /**
   * Resolve the device a Flow card must act on.
   *
   * Flow cards are app-wide singletons: `registerRunListener()` holds exactly one
   * listener per card and every call overwrites the previous one. Because this
   * service is instantiated per device, a handler that closes over `this.device`
   * would operate on whichever device happened to register last — not on the
   * device the user selected in the card.
   *
   * Every Flow card in this app declares a `device` argument, so `args.device` is
   * the authoritative target. The fallback to `this.device` only covers a card
   * invoked without a device argument, which should not occur.
   */
  private resolveDevice(args: unknown): Homey.Device {
    const candidate = (args as { device?: Homey.Device } | undefined)?.device;
    return candidate ?? this.device;
  }

  /**
   * Resolve the ServiceCoordinator belonging to a specific device.
   * Returns undefined when the device has not finished initializing.
   */
  private static coordinatorOf(device: Homey.Device): DeviceServiceCoordinator | undefined {
    return (device as unknown as { serviceCoordinator?: DeviceServiceCoordinator }).serviceCoordinator;
  }

  /**
   * Resolve the BuildingInsightsService of a specific device, falling back to the
   * instance this service was constructed with.
   */
  private buildingInsightsOf(device: Homey.Device): BuildingInsightsService | undefined {
    return FlowCardManagerService.coordinatorOf(device)?.getBuildingInsights?.()
      ?? this.buildingInsightsService;
  }
  ...
  async updateFlowCards(capabilitiesWithData?: string[]): Promise<void> {
    try {
      ...
      // Flow cards are app-wide singletons: registerRunListener() holds exactly one
      // listener per card, and a later call overwrites it. Registering from every
      // device instance is therefore redundant — one registrar serves all devices,
      // because each handler resolves its target through args.device.
      //
      // NOTE: the per-category conditional registration that used to live here
      // (registerFlowCardsByCategory, driven by the flow_* settings) has been
      // removed. It could not work: "register this card for device A but not for
      // device B" has no meaning for a singleton. The methods it called registered
      // nothing anyway — trigger cards come from the pattern-based system in
      // app.ts — so the flow_* settings never affected Flow card visibility,
      // despite what their labels promise.
      //
      // The settings are NOT unused: CapabilityHealthService.isCategoryEnabled()
      // reads them to decide which capability categories take part in health
      // tracking. Do not delete them without adjusting that service.
      // See docs/architecture/IMPLEMENTATIEPLAN-v3.md, besluit B3 and fase 8.
      ...
      if (isRegistrar) {
        ...
        // Clear the internal bookkeeping map before re-registering.
        ...
        this.logger(
          'FlowCardManagerService: Flow card listeners already registered app-wide by another device — skipping registration',
        );
      }

      // Per-device work that must run for every device, registrar or not:
      // the performance report scheduler owns device-scoped timers.
  ...
  private async registerActionBasedConditionCards(): Promise<void> {
    try {
      ...
      const devicePowerListener = devicePowerCard.registerRunListener(async (args) => {
        ...
        const currentValue = device.getCapabilityValue('onoff');
      ...
      const hotWaterTempListener = hotWaterTempCard.registerRunListener(async (args) => {
        ...
        const device = this.resolveDevice(args);
        const currentValue = device.getCapabilityValue('adlar_hotwater') || 0;
        const targetValue = args.temperature || 0;
      ...
      const heatingModeListener = heatingModeCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_mode');
      ...
      const workModeListener = workModeCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_work_mode');
        return currentValue === args.mode;
      });
      ...
      const waterModeListener = waterModeCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_water_mode') || 0;
      ...
      const capacitySettingListener = capacitySettingCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_capacity_set');
      ...
      const heatingCurveListener = heatingCurveCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_countdown_set');
      ...
      const volumeSettingListener = volumeSettingCard.registerRunListener(async (args) => {
        ...
        const currentValue = this.resolveDevice(args).getCapabilityValue('adlar_enum_volume_set') || 0;
      ...
      const copEfficiencyListener = copEfficiencyCard.registerRunListener(async (args) => {
        ...
        const currentCOP = device.getCapabilityValue('adlar_cop') as number || 0;
        const threshold = args.threshold || 2.0;

        // Check if compressor is actually running (COP only meaningful when active)
        const compressorFrequency = device.getCapabilityValue('measure_frequency.compressor_strength') as number || 0;
      ...
      const dailyCOPListener = dailyCOPCard.registerRunListener(async (args) => {
        ...
        const dailyCOP = this.resolveDevice(args).getCapabilityValue('adlar_cop_daily') as number || 0;
      ...
      const monthlyCOPListener = monthlyCOPCard.registerRunListener(async (args) => {
        ...
        const monthlyCOP = this.resolveDevice(args).getCapabilityValue('adlar_cop_monthly') as number || 0;
      ...
      const copTrendListener = copTrendCard.registerRunListener(async (args) => {
        ...
        const serviceCoordinator = FlowCardManagerService.coordinatorOf(this.resolveDevice(args));
        ...
        // Optional call: ServiceCoordinator does not currently expose this accessor
        // (RollingCOPCalculator is a private field on the device class). Calling it
        // unconditionally threw a TypeError and aborted the Flow.
  ...
  private async registerBuildingInsightsCards(): Promise<void> {
    ...
    try {
      ...
      const forceAnalysisListener = forceAnalysisCard.registerRunListener(async (args) => {
        this.logger('FlowCardManagerService: Force insight analysis action triggered');

        const buildingInsights = this.buildingInsightsOf(this.resolveDevice(args));
        if (!buildingInsights) {
          throw new Error('Building Insights service not available');
        }

        const result = await buildingInsights.forceInsightAnalysis();
        this.logger(
      ...
      const calculatePreHeatListener = calculatePreHeatCard.registerRunListener(async (args: {
        ...
        if (!this.buildingInsightsOf(device)) {
          throw new Error('Building Insights service not available');
        }

        // Get building model parameters via BuildingModelService
        const buildingModelService = FlowCardManagerService.coordinatorOf(device)
          ?.getAdaptiveControl?.()
          ?.getBuildingModelService?.();
        ...
        const indoorTemp: number | null = FlowCardManagerService.coordinatorOf(device)
          ?.getAdaptiveControl?.()
          ?.getExternalTemperatureService?.()
  ...
  private async registerPerformanceReportCard(): Promise<void> {
    try {
      ...
      const generateReportListener = generateReportCard.registerRunListener(async (args) => {
        this.logger('FlowCardManagerService: Generate performance report action triggered');
        return this.generateAndStoreReport(this.resolveDevice(args));
      });
      this.flowCardListeners.set('generate_performance_report', generateReportListener);

      this.logger('FlowCardManagerService: Performance Report action card registered');
    } catch (error) {
  ...
  /**
   * Schedule the device-scoped performance report timers.
   *
   * Split out of registerPerformanceReportCard() because these timers belong to
   * *this* device, whereas Flow card registration is app-wide and performed by a
   * single registrar instance. Every device must schedule its own timers, also
   * when another device registered the cards.
   */
  private schedulePerformanceReportTimers(): void {
    try {
      // ── Trigger Card: Performance Report Ready (daily) ──
      // Store as a dedicated class property — NOT in flowCardListeners, so that
      // clearing that map does not disturb the 23:00 timer lifecycle.
      ...
      this.logger('FlowCardManagerService: Error scheduling performance report timers:', error);
    }
  }
   ...
   */
  /**
   * Generate a performance report for a specific device and store it in that
   * device's capabilities.
   *
   * @param device - target device; defaults to the device this service belongs to,
   *   used by the internal schedulers. The Flow card handler passes args.device so
   *   the report is generated for the device the user selected, not for whichever
   *   device happened to register the card.
   */
  private async generateAndStoreReport(device: Homey.Device = this.device): Promise<PerformanceReportFlowTokens> {
    ...
    if (device.hasCapability('adlar_performance_score')) {
      await device.setCapabilityValue('adlar_performance_score', report.overallScore);
    }
    if (device.hasCapability('adlar_performance_report')) {
      await device.setCapabilityValue('adlar_performance_report', reportJson);
    }
    ...
    try {
      ...
      const receiveExternalPowerListener = receiveExternalPowerCard.registerRunListener(
        ...
        async (args: { power_value: number }) => {
          ...
          const device = this.resolveDevice(args);
          await this.handleReceiveExternalPowerData(device, args);

          // Trigger intelligent power update if enabled
          const energyTrackingEnabled = device.getSetting('enable_intelligent_energy_tracking');
          if (energyTrackingEnabled) {
            // @ts-expect-error - updateIntelligentPowerMeasurement exists in MyDevice
            await device.updateIntelligentPowerMeasurement?.();
          }
          return true;
        },
      );
      ...
      const receiveExternalFlowListener = receiveExternalFlowCard.registerRunListener(
        ...
        async (args: { flow_value: number }) => {
          ...
          await this.handleReceiveExternalFlowData(this.resolveDevice(args), args);
          return true;
        },
      );
      ...
      const receiveExternalAmbientListener = receiveExternalAmbientCard.registerRunListener(
        ...
        async (args: { temperature_value: number }) => {
          ...
          await this.handleReceiveExternalAmbientData(this.resolveDevice(args), args);
          return true;
        },
      );
      ...
      const receiveExternalIndoorListener = receiveExternalIndoorCard.registerRunListener(
        ...
        async (args: { temperature_value: number | string }) => {
          ...
          const adaptiveControl = FlowCardManagerService
            .coordinatorOf(this.resolveDevice(args))
            ?.getAdaptiveControl?.();
          if (adaptiveControl?.receiveExternalTemperature) {
      ...
      const receiveExternalPricesListener = receiveExternalPricesCard.registerRunListener(
        ...
        async (args: { prices_json: string }) => {
          ...
          await this.handleReceiveExternalEnergyPrices(this.resolveDevice(args), args);
          return true;
        },
      );
      ...
      const receiveExternalWindListener = receiveExternalWindCard.registerRunListener(
        ...
        async (args: { wind_speed: number }) => {
          ...
          await this.handleReceiveExternalWindData(this.resolveDevice(args), args);
          return true;
        },
      );
      ...
      const receiveExternalSolarPowerListener = receiveExternalSolarPowerCard.registerRunListener(
        ...
        async (args: { power_value: number }) => {
          ...
          await this.handleReceiveExternalSolarPower(this.resolveDevice(args), args);
          return true;
        },
      );
      ...
      const receiveExternalSolarRadiationListener = receiveExternalSolarRadiationCard.registerRunListener(
        ...
        async (args: { radiation_value: number }) => {
          ...
          await this.handleReceiveExternalSolarRadiation(this.resolveDevice(args), args);
          return true;
        },
      );
  ...
  private unregisterAllFlowCards(): void {
    this.flowCardListeners.forEach((listener, cardId) => {
      try {
        // Homey's FlowCard has no unregister API — registerRunListener() returns the
        // FlowCard itself, not a subscription handle. This guard is therefore never
        // true; it is kept only in case a future SDK version adds one. The previous
        // log line claimed cards were unregistered, which was never the case and
        // misled debugging.
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalAmbientData(device: Homey.Device, args: { temperature_value: number }): Promise<void> {
    try {
      if (device.hasCapability('adlar_external_ambient')) {
        await device.setCapabilityValue('adlar_external_ambient', args.temperature_value); // eslint-disable-line camelcase
        ...
        await device.setStoreValue('external_outdoor_temp', args.temperature_value);
        await device.setStoreValue('external_outdoor_temp_timestamp', Date.now());

        // Update diagnostic capability
        if (device.hasCapability('adlar_last_outdoor_temp_received')) {
          const d = new Date();
          const ts = this.formatDiagnosticTimestamp(d);
          await device.setCapabilityValue('adlar_last_outdoor_temp_received', `${ts} | ${args.temperature_value.toFixed(1)}°C`); // eslint-disable-line camelcase
        }

        // Emit event for other services
        device.emit('external-data:ambient', args.temperature_value);
      }
    } catch (error) {
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalFlowData(device: Homey.Device, args: { flow_value: number }): Promise<void> {
    try {
      if (device.hasCapability('adlar_external_flow')) {
        await device.setCapabilityValue('adlar_external_flow', args.flow_value); // eslint-disable-line camelcase
        this.logger(`FlowCardManagerService: External flow data updated: ${args.flow_value} L/min`); // eslint-disable-line camelcase

        // Emit event for other services
        device.emit('external-data:flow', args.flow_value);
      }
    } catch (error) {
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalPowerData(device: Homey.Device, args: { power_value: number }): Promise<void> {
    try {
      if (device.hasCapability('adlar_external_power')) {
        await device.setCapabilityValue('adlar_external_power', args.power_value); // eslint-disable-line camelcase
        ...
        if (device.hasCapability('defrost_active_power')) {
          const isDefrosting = device.getCapabilityValue('adlar_state_defrost_state') || false;
          const newValue = isDefrosting ? args.power_value : 0; // eslint-disable-line camelcase
          await device.setCapabilityValue('defrost_active_power', newValue);
        }
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalEnergyPrices(device: Homey.Device, args: { prices_json: string }): Promise<void> {
    try {
      ...
      try {
        ...
        device.error('❌ FlowCardManagerService: Failed to parse prices JSON:', error);
      ...
      const energyOptimizer = device.serviceCoordinator?.getAdaptiveControl()?.getEnergyPriceOptimizer();
      if (!energyOptimizer) {
      ...
      if (device.hasCapability('energy_prices_data')) {
        ...
        await device.setCapabilityValue('energy_prices_data', JSON.stringify(richData));
      ...
      device.error('❌ FlowCardManagerService: Error receiving external energy prices:', error);
      throw error;
    }
  }
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalWindData(device: Homey.Device, args: { wind_speed: number }): Promise<void> {
    try {
      ...
      // Update capability
      if (device.hasCapability('adlar_external_wind_speed')) {
        await device.setCapabilityValue('adlar_external_wind_speed', windSpeed);
        this.logger(`FlowCardManagerService: External wind speed updated: ${windSpeed} km/h`);
      }

      // Store for persistence
      await device.setStoreValue('external_wind_speed', windSpeed);
      await device.setStoreValue('external_wind_speed_timestamp', Date.now());

      // Update diagnostic capability
      if (device.hasCapability('adlar_last_wind_received')) {
        const d = new Date();
        const ts = this.formatDiagnosticTimestamp(d);
        await device.setCapabilityValue('adlar_last_wind_received', `${ts} | ${windSpeed.toFixed(0)}km/h`);
      }
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalSolarPower(device: Homey.Device, args: { power_value: number }): Promise<void> {
    try {
      ...
      // Update capability
      if (device.hasCapability('adlar_external_solar_power')) {
        await device.setCapabilityValue('adlar_external_solar_power', powerValue);
        this.logger(`FlowCardManagerService: External solar power updated: ${powerValue} W`);
      }

      // Store for persistence
      await device.setStoreValue('external_solar_power', powerValue);
      await device.setStoreValue('external_solar_power_timestamp', Date.now());

      // Update diagnostic capability
      if (device.hasCapability('adlar_last_solar_power_received')) {
        const d = new Date();
        const ts = this.formatDiagnosticTimestamp(d);
        await device.setCapabilityValue('adlar_last_solar_power_received', `${ts} | ${powerValue.toFixed(0)}W`);
      }
   ...
   */
  // eslint-disable-next-line camelcase
  async handleReceiveExternalSolarRadiation(device: Homey.Device, args: { radiation_value: number }): Promise<void> {
    try {
      ...
      // Update capability
      if (device.hasCapability('adlar_external_solar_radiation')) {
        await device.setCapabilityValue('adlar_external_solar_radiation', radiationValue);
        this.logger(`FlowCardManagerService: External solar radiation updated: ${radiationValue} W/m²`);
      }

      // Store for persistence
      await device.setStoreValue('external_solar_radiation', radiationValue);
      await device.setStoreValue('external_solar_radiation_timestamp', Date.now());

      // Update diagnostic capability
      if (device.hasCapability('adlar_last_solar_radiation_received')) {
        const d = new Date();
        const ts = this.formatDiagnosticTimestamp(d);
        await device.setCapabilityValue('adlar_last_solar_radiation_received', `${ts} | ${radiationValue.toFixed(0)}W/m²`);
      }
  ...
  destroy(): void {
    ...
    // Release the app-wide registrar role so another device can take it over.
    // The listeners themselves cannot be removed (no SDK API); they are replaced
    // as soon as the next device calls updateFlowCards().
    if (FlowCardManagerService.registrar === this) {
      FlowCardManagerService.registrar = null;
      this.logger('FlowCardManagerService: Released app-wide flow card registrar role');
    }
